...
async function previewPlay(lickId,abc,tempo,feel){
  ...
  if(_chordSamplerReady&&_chordSampler&&chordTimes.length>0){
    var cRev=new Tone.Reverb({decay:1.5,wet:0.12}).toDestination();
    _chordSampler.disconnect();_chordSampler.connect(cRev);
    chordCleanup=function(){try{_chordSampler.releaseAll();_chordSampler.disconnect();_chordSampler.toDestination();}catch(e){}try{cRev.dispose();}catch(e){}};
  }
  if(_samplerReady&&_sampler){
    ...
    // Schedule chords on same time base
    if(_chordSamplerReady&&_chordSampler&&chordTimes.length>0){
      for(var ci=0;ci<chordTimes.length;ci++){var ch=chordTimes[ci];var cn=chordToNotes(ch.name,4);if(!cn.length)continue;
    ...
    // Schedule chords in fallback too
    if(_chordSamplerReady&&_chordSampler&&chordTimes.length>0){
      for(var ci2=0;ci2<chordTimes.length;ci2++){var ch2=chordTimes[ci2];var cn2=chordToNotes(ch2.name,4);if(!cn2.length)continue;
...
function DailyLickCard({lick,onSelect,th,liked,saved,onLike,onSave,userInst:userInst,onArtistSearch}){
  ...
  return React.createElement("div",{"data-coach":"daily",onClick:()=>onSelect(lick),style:{background:isStudio?(t.cardRaised||t.card):t.card,borderRadius:isStudio?20:16,padding:0,marginBottom:isStudio?18:14,border:"1px solid "+(isStudio?catC+"25":t.border),borderLeft:isStudio?"none":("3px solid "+instBorderC),cursor:"pointer",boxShadow:isStudio?"0 4px 24px "+catC+"20, 0 1px 8px rgba(0,0,0,0.4), inset 0 1px 0 "+catC+"10":"0 2px 12px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.03)",transition:"box-shadow 0.2s, transform 0.15s",overflow:"hidden",display:"flex"}},
    ...
    React.createElement("div",{style:{flex:1,padding:isStudio?18:16}},
      ...
      // NOTATION — single line with fade
      React.createElement("div",{style:{marginTop:6,position:"relative",overflow:"hidden"}},
        ...
        React.createElement("div",{style:{position:"absolute",top:0,right:0,bottom:0,width:32,background:"linear-gradient(to right, transparent, "+(isStudio?t.cardRaised||t.card:t.card)+")",pointerEvents:"none"}})),
...
function LickCard({lick,onSelect,th,liked,saved,onLike,onSave,userInst:userInst,onUserClick,onArtistSearch,animIdx}){
  ...
  return React.createElement(React.Fragment,null,
    ...
    React.createElement("div",{style:{flex:1,padding:isStudio?16:14}},
      ...
      // NOTATION — single line with fade
      React.createElement("div",{style:{marginTop:4,position:"relative",overflow:"hidden"}},
        React.createElement("div",{style:{display:"flex",justifyContent:"center"}},
