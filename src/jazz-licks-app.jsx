...
// ABC PARSER
// ============================================================
function parseAbc(abcStr,tOv){const lines=abcStr.split("\n");let dL=1/8,bpm=120,ks={},tsN=4;for(const l of lines){const t=l.trim();if(t.startsWith("L:")){const m=t.match(/(\d+)\/(\d+)/);if(m)dL=parseInt(m[1])/parseInt(m[2]);}else if(t.startsWith("Q:")){const m=t.match(/(\d+)$/);if(m)bpm=parseInt(m[1]);}else if(t.startsWith("K:")){ks=KEY_SIG[t.replace("K:","").trim().split(/\s/)[0]]||{};}else if(t.startsWith("M:")){const m=t.match(/(\d+)\/(\d+)/);if(m)tsN=parseInt(m[1]);}}if(tOv)bpm=tOv;const spb=60/bpm;let mu="";for(const l of lines){const t=l.trim();if(/^[A-Z]:/.test(t))continue;mu+=" "+t;}const ev=[],ch=[];let i=0;var tupRem=0,tupFac=1;while(i<mu.length){const c=mu[i];if(c===" "||c==="\t"){i++;continue;}if(c==="|"||c===":"){i++;continue;}if(c==="]"){i++;continue;}if(c==='"'){i++;let cn="";while(i<mu.length&&mu[i]!=='"'){cn+=mu[i];i++;}if(i<mu.length)i++;let p=0;for(const e of ev)p+=e.rL;ch.push({name:cn,pos:p});continue;}if(c==="!"||c==="+"){i++;while(i<mu.length&&mu[i]!==c)i++;if(i<mu.length)i++;continue;}if(c==="("){i++;let tn="";while(i<mu.length&&mu[i]>="0"&&mu[i]<="9"){tn+=mu[i];i++;}var tp=parseInt(tn)||3;tupRem=tp;tupFac=tp===3?2/3:tp===5?4/5:tp===7?6/7:2/3;continue;}if(c==="~"||c==="."||c==="H"){i++;continue;}if(c==="-"){if(ev.length>0)ev[ev.length-1].tie=true;i++;continue;}if(c==="["){if(i+1<mu.length&&(mu[i+1]==="|"||mu[i+1]==="1"||mu[i+1]==="2")){i++;continue;}i++;const ct=[];while(i<mu.length&&mu[i]!=="]"){const nr=rN(mu,i,ks);if(nr){ct.push(nr.t);i=nr.n;}else i++;}if(i<mu.length)i++;const dr=rD(mu,i);i=dr.n;var rl3=dL*dr.m;if(tupRem>0){rl3*=tupFac;tupRem--;}ev.push({tn:ct.length?ct:null,rL:rl3});continue;}if(c==="z"||c==="x"){i++;const dr=rD(mu,i);i=dr.n;var rl=dL*dr.m;if(tupRem>0){rl*=tupFac;tupRem--;}ev.push({tn:null,rL:rl});continue;}const nr=rN(mu,i,ks);if(nr){i=nr.n;const dr=rD(mu,i);i=dr.n;var rl2=dL*dr.m;if(tupRem>0){rl2*=tupFac;tupRem--;}ev.push({tn:[nr.t],rL:rl2});continue;}i++;}// Merge tied notes: if event[i].tie && event[i+1] has same pitch, combine durations
